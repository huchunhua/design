    ...
    $(window).on( 'scroll' , function () {
        if (! checkScrollSlide() ) {
            ...
            $.each(ar, function( key , value) {   
                ...
                $(oImg).css('width' , '214px');
                $(oImg).css('height' , value.height / value.width * 214 +'px');
                $(oImg).attr('src' , '');
                var img = new Image();
                img.onload = function() {
                ...
                var oCoverh = $('<div>');
                $(oCoverh).addClass('coverh');
                var oCai = $('<div>');
                $(oCai).html('采集');
                var oLikewrap = $('<div>');
                $(oLikewrap).append($('<i>').addClass('like'));
                $(oLikewrap).addClass('like-wrap').appendTo($(oCoverh));
                $(oCai).addClass('cai').appendTo($(oCoverh));
                var oPic = $('<div>').addClass('pic').append($(oImg));
                $(oCoverh).appendTo($(oPic));
                var oBox = $('<div>').addClass('box').append($(oPic)); 
                $(oBox).appendTo($('#waterfall'));
                waterfall();
            })
        }
    })
...
function waterfall() {
    ...
    var cols =Math.floor($('#waterfall').width() / w);
    $('#waterfall').width(w * cols);
    $('#watwefall').css('margin','0 auto');
    var hArr = [];
